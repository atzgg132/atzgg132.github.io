import {
  $, $$, store, booking, toast, go, back, closeSheets, sheet, INR,
  isDemoOtp, unlockDemo, saveSession, saveBooking, TABS, ICONS, DIYA_AVATAR,
} from './core.js';
import {
...
function startTimer() {
  ...
  store.otp.timer = store.data?.auth?.otpResendSeconds || 30;
  const wait = $("#otp-wait");
...
function bindOtp(form) {
  ...
  const fill = (digits) => {
    ...
    inputs.forEach((inp, idx) => { inp.value = d[idx] || ""; });
    return d;
  };
  ...
  inputs.forEach((inp, i) => {
    inp.addEventListener("input", (e) => {
      ...
      if (value().length === 6) setTimeout(() => form.requestSubmit(), 120);
    });
  ...
  form.onsubmit = (e) => {
    ...
    btn.disabled = true;
    setTimeout(() => {
      // Demo login only unlocks Your Trips. 000000 always works; any 6 digits also pass.
      const ok = isDemoOtp(code) || code.length === 6;
      if (!ok) {
      ...
      unlockDemo(store.otp.mobile || "8391081502");
...
function installCopy() {
  ...
  if (ios && safari) return "Tap the Share button, then Add to Home Screen.";
  if (ios) return "Open this site in Safari, then Share → Add to Home Screen.";
  return "Use the browser menu → Install app / Add to Home screen.";
}

async function installApp() {
  if (store.deferredInstall?.prompt) {
    try {
      store.deferredInstall.prompt();
      await store.deferredInstall.userChoice;
    } catch (_) {}
  ...
  sheet(`<h3>Add to Home Screen</h3>
    ...
    <p class="note">On iPhone this only appears in <b>Safari</b> (not Chrome or in-app browsers).</p>
...
async function boot() {
  ...
  if (localStorage.getItem("yatra.build") !== BUILD) {
    localStorage.setItem("yatra.build", BUILD);
    try {
  ...
  store.data = await fetch("data/app.json?v=" + BUILD).then((r) => r.json());
  document.querySelectorAll(".statusbar").forEach((n) => n.remove());
