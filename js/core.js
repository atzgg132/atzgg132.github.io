...
const DEMO_MOBILES = ["8391081502", "000000", "0000000000"];
...
function isMagicMobile(n) {
  const d = digits(n);
  const allow = [...DEMO_MOBILES, ...(store.data?.auth?.magicMobiles || [])]
    .map(digits)
    .filter(Boolean);
  return allow.some((m) => d === m || (m && d && (d.endsWith(m) || m.endsWith(d))));
}

function isDemoOtp(code) {
  return digits(code) === DEMO_OTP;
}

function unlockDemo(mobile) {
  store.session = {
    mobile: digits(mobile) || DEMO_MOBILES[0],
    name: store.data?.user?.name || "Arnav Bhattacharya",
    at: Date.now(),
  };
  saveSession();
}
...
export {
  ...
  saveSession, saveBooking, booking, isMagicMobile, isDemoOtp, unlockDemo,
  DEMO_OTP, DEMO_MOBILES, formatStamp,
